...
from .backbone.backbone_3d.backbone_3d import Backbone3D
from .backbone.yowo.encoder import build_channel_encoder
...
class Feature_Fusion(nn.Module):
    def __init__(self, depth = 1.0, width = 1.0, in_features = ("dark3", "dark4", "dark5"), in_channels = [128,128,128], depthwise = False, act = "silu"):
        super().__init__()
        Conv                = DWConv if depthwise else BaseConv
        #self.backbone       = CSPDarknet(depth, width, depthwise = depthwise, act = act)
        self.in_features    = in_features
...
class STNetwork(nn.Module):
    def __init__(self, num_classes, fp16=False, num_frame=10):
        ...
        self.cls_channel_encoders1=build_channel_encoder(mcfg, 128+bk_dim_3d, mcfg['head_dim'])
        self.cls_channel_encoders2=build_channel_encoder(mcfg, 256+bk_dim_3d, mcfg['head_dim'])
        self.cls_channel_encoders3=build_channel_encoder(mcfg, 512+bk_dim_3d, mcfg['head_dim'])
        ...
        #         nn.LeakyReLU()) 
        self.feature_fusion=Feature_Fusion()#depth =0.33, width =1.0
