...
class CSPDarknet(nn.Module):
    ...
    def forward(self, x):
        ...
        outputs["stem"] = x 
        x = self.dark2(x)  #(4,64,128,128)
        outputs["dark2"] = x
        #-----------------------------------------------#
        #   dark3的输出为80, 80, 256，是一个有效特征层
        #-----------------------------------------------#
        x = self.dark3(x)  #(4,128,64,64)
        outputs["dark3"] = x 
        #-----------------------------------------------#
        #   dark4的输出为40, 40, 512，是一个有效特征层
        #-----------------------------------------------#
        x = self.dark4(x) #(4,256,32,32)
        outputs["dark4"] = x
        #-----------------------------------------------#
        #   dark5的输出为20, 20, 1024，是一个有效特征层
        #-----------------------------------------------#
        x = self.dark5(x) #(4,512,16,16)
        outputs["dark5"] = x
...
if __name__ == '__main__':
    backbone    = CSPDarknetTSM(1.0, 1.0, out_features=("dark3", "dark4", "dark5"),depthwise =False, act ="silu",segment=5,div=5).cuda()
    input=torch.randn(4,3,5,512,512).cuda()
    input=input.view((-1, 3) + input.size()[-2:]) #(20,3,512,512)
