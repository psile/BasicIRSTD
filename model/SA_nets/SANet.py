...
class SANet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = nn.Upsample(scale_factor=2, mode='nearest')(x)  # 至[256, 64, 64]
        x = nn.Upsample(scale_factor=2, mode='nearest')(x)  # 至[256, 128, 128]
        x = nn.Upsample(scale_factor=2, mode='nearest')(x)  # 至[256, 256, 256]
    
        # 减少通道数至1
        x =self.conv1(x)  # 将通道数从256减少到1
