...
def test(): 
    ...
    #     net1 = Net(model_name="AGPCNet", mode='test').cuda()
    #     try:
    #         net1.load_state_dict(torch.load("log/"+pth_dir)['state_dict'])
    #     except:
    #         device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    #         net1.load_state_dict(torch.load("log/"+pth_dir, map_location=device)['state_dict'])
    #     net1.eval()
    # elif model_name=="DNANet":
    #     net2 = Net(model_name="DNANet", mode='test').cuda()
    #     try:
    #         net2.load_state_dict(torch.load("log/"+pth_dir)['state_dict'])
    #     except:
    #         device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    #         net2.load_state_dict(torch.load("log/"+pth_dir, map_location=device)['state_dict'])
    #     net2.eval()
    # elif model_name=="SSTNet":
    #     net3 = Net(model_name="SSTNet", mode='test').cuda()
    #     try:
    #         net3.load_state_dict(torch.load("log/"+pth_dir)['state_dict'])
    #     except:
    #         device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    #         net3.load_state_dict(torch.load("log/"+pth_dir, map_location=device)['state_dict'])
    #     net.eval()
