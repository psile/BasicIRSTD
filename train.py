...
parser.add_argument("--model_names", default=['ACM'], type=list, 
                    help="model_name: 'ACM', 'SANet','ALCNet', 'DNANet', 'ISNet', 'UIUNet', 'RDIAN', 'ISTDU-Net', 'U-Net', 'RISTDnet'")      #,'ACM', 'ALCNet         
...
def train():
    ...
    if opt.optimizer_name == 'Adam':
        ...
        opt.scheduler_settings = {'epochs':400, 'step': [200, 300], 'gamma': 0.1}#400 
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        for idx_iter, (img, gt_mask) in enumerate(train_loader):#train_loader tq.tqdm( ,desc=f"Epoch {idx_epoch+1}/{opt.nEpochs}",leave=False)
        ...
        if (idx_epoch + 1) == opt.nEpochs and (idx_epoch + 1) % 10 != 0: #50
            save_pth = opt.save + '/' + opt.dataset_name + '/' + opt.model_name + '_' + str(idx_epoch + 1) + '.pth.tar'
            save_checkpoint({
                'epoch': idx_epoch + 1,
                'state_dict': net.state_dict(),
